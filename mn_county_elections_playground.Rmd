...
#  filter(district == 1) %>%
  ...
  full_join(comm_dists_19_area_final %>% select(!geometry),
            by = c("county_id", "county_name", "county_num_precincts", "region")) 

comm2000.2018 %>% 
  ggplot(aes(x = candidates, y = candidate_percent_vote, color = fifth)) +
                           geom_jitter()
...
county <- 80
  ...
  dists <- unlist(table_county_repeats.save_elections2 %>% 
    ...
    df_num2 <- df_num %>% filter(!won == FALSE)
