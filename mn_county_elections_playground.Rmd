...
northwest <- c(35, 68, 39, 45, 4, 57, 60, 63, 15, 54, 44, 14, 3, 29)
northeast <- c(36, 69, 38, 16, 31, 1, 9, 58, 33, 11, 18)
central <- c(6, 76, 34, 47, 86, 73, 5, 48, 61, 75, 78, 84, 56, 26, 21, 80, 77, 49)
metro <- c(71, 30, 13, 2, 82, 62, 27, 10, 70, 19)
southwest <- c(12, 65, 64, 17, 32, 53, 67, 59, 51, 41, 42, 87, 37)
south_central <- c(43, 72, 52, 8, 83, 46, 22, 7, 40, 81)
southeast <- c(66, 25, 79, 74, 24, 20, 50, 55, 85, 23, 28)
...
county_ids <- read.delim("relevant_county_data/county_ids.txt", 
  ...
  mutate(region = case_when(county_id %in% northwest ~ "Northwest",
                            county_id %in% northeast ~ "Northeast",
                            county_id %in% central ~ "Central",
                            county_id %in% metro ~ "Metro",
                            county_id %in% southwest ~ "Southwest",
                            county_id %in% south_central ~ "South Central",
                            county_id %in% southeast ~ "Southeast",
                            TRUE ~ "other"
                            )) %>% 
  mutate(num_ctycomdists = ifelse(county_name %in% c("Anoka", "Dakota", "Hennepin", 
                                                     "Olmsted", 'Ramsey', "St. Louis"),
                                  7, 5))
...
was.comm.election <- do.call(rbind, datalist2000.2018) %>% 
  ...
  mutate(e2000 = ifelse(e2000 == FALSE, NA, e2000),
         e2002 = ifelse(e2002 == FALSE, NA, e2002),
         e2004 = ifelse(e2004 == FALSE, NA, e2004),
         e2006 = ifelse(e2006 == FALSE, NA, e2006),
         e2008 = ifelse(e2008 == FALSE, NA, e2008),
         e2010 = ifelse(e2010 == FALSE, NA, e2010),
         e2012 = ifelse(e2012 == FALSE, NA, e2012),
         e2014 = ifelse(e2014 == FALSE, NA, e2014),
         e2016 = ifelse(e2016 == FALSE, NA, e2016),
         e2018 = ifelse(e2018 == FALSE, NA, e2018))

was.comm.election %>% filter(equal_nums == FALSE) %>% View
...
## Playing with plots from table with all county commissioner candidate data

years.vector <- c(2000, 2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016, 2018)

commlist2000.2018 = list()

for(i in 1:10){
  dat <- clean_gen_comm_candidates(years.vector[i]) %>% 
    mutate(year = years.vector[i])
  commlist2000.2018[[i]] <- dat
}

comm2000.2018 <- do.call(rbind, commlist2000.2018) %>% 
  filter(office_name == "Commissioner") %>%
      full_join(do.call(rbind, commlist2000.2018) %>% 
                filter(office_name == "Commissioner") %>%
                group_by(district, county_id, year) %>% 
                ...
                ungroup(), by = c("county_id", "office_id", "office_name", "district",
                                  ...
                                  "other_county_id", "year")) %>% 
  mutate(winner = ifelse(is.na(winner), FALSE, winner)) %>%
  group_by(district, year, county_id) %>% 
  mutate(candidates = n() - 1) %>% 
  ungroup() %>%  
  select(county_id, district, year, candidate_name, candidate_votes,
         candidate_percent_vote, candidates, winner, special) %>% 
  filter(winner == TRUE) %>% 
  select(!winner) %>% 
  full_join(county_ids, by = "county_id") %>% 
  mutate(county_id = as.factor(county_id),
         district = as.factor(district), 
         candidates = as.factor(candidates)) %>% 
  full_join(comm_dists_19_area_final %>% select(!geometry) %>% rename(district = ctycomdist),
            by = c("county_id", "district", "county_name", "county_num_precincts", "region")) 

comm2000.2018_all <- do.call(rbind, commlist2000.2018) %>% 
  filter(office_name == "Commissioner") %>%
      full_join(do.call(rbind, commlist2000.2018) %>% 
                filter(office_name == "Commissioner") %>%
                group_by(district, county_id, year) %>% 
  ...
  mutate(county_id = as.factor(county_id),
         ...
         candidates = as.factor(candidates)) %>% 
  full_join(comm_dists_19_area_final %>% select(!geometry) %>% rename(district = ctycomdist),
            by = c("county_id", "district", "county_name", "num_ctycomdists",
                   "county_num_precincts", "region")) 
...
nrow(comm2000.2018_all %>% filter(candidate_percent_vote >= 2))
sample
```
...
## Creating wide table showing repeat candidates for all counties 
...
## Gathering winners and candidates for all years, for upcoming table
...
table_county_repeats.list2 = list()

for(i in 1:87){
  dat <- comm_candidate_gen_repeats_table(i)
  table_county_repeats.list2[[i]] <- dat
}

table_county_repeats2 <- do.call(bind_rows, table_county_repeats.list2)

table_county_repeats.save2 <- table_county_repeats2 %>% 
 ...
 # saveRDS(table_county_repeats.save2, file = "relevant_county_data/table_county_repeats_all.rds")
...
## Reshaping table of winners with commmissioner as column

table_county_repeats.save2 <- readRDS("relevant_county_data/table_county_repeats_all.rds")
...
table_county_repeats.save_edit2 <- table_county_repeats.save2 %>% 
  arrange(county_id, district) %>%
  select(county_id:winner_2018)

table_county_repeats.save_elections2 <- 
  bind_rows(was.comm.election_edit, table_county_repeats.save_edit2) %>% 
  arrange(county_id, district)

matrix.please<-function(x) {
    m <- data.matrix(x[,-1:-2])
    rownames(m) <- as.vector(unlist(x[,1]))
    m
}


table_county_repeats.save_elections2 %>% 
  filter(county_id == 59, district == 2)
...
## Gather number of commissioners over time

county_dist_stats = list()

for(k in 1:87){

  county <- k
  
  dists <- unlist(table_county_repeats.save_elections2 %>% 
                    filter(county_id == county) %>% 
                    distinct(district))

  county_dist_stats_list = list()
  
  for(i in 1:length(dists)){
    df <- table_county_repeats.save_elections2 %>% 
      filter(county_id == county) %>%
      filter(district == dists[i]) %>% 
      pivot_longer(!c(county_id, district, candidate_name), names_to = "years") %>% 
      pivot_wider(names_from = candidate_name, values_from = value) %>% 
      mutate(years = as.numeric(str_remove_all(years, "winner_"))) %>% 
      filter(ELECTION == TRUE) %>% 
      select(!district) %>% 
      select(!county_id)
    df2 <- df[, (colSums(df, na.rm = TRUE) != 0)]
    
    if(ncol(df2) - 2 == 1){
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]))

          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)

    }else{if(ncol(df2) - 2 == 2){
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   "two" = colnames(df2[,2 + 2]),
                   )

          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)

    }else{if(ncol(df2) - 2 == 3){
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   "two" = colnames(df2[,2 + 2]),
                   "three" = colnames(df2[,2 + 3]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)

    }else{if(ncol(df2) - 2 == 4){
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   "two" = colnames(df2[,2 + 2]),
                   "three" = colnames(df2[,2 + 3]),
                   "four" = colnames(df2[,2 + 4]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three, four), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)

    }else{if(ncol(df2) - 2 == 5){
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   "two" = colnames(df2[,2 + 2]),
                   "three" = colnames(df2[,2 + 3]),
                   "four" = colnames(df2[,2 + 4]),
                   "five" = colnames(df2[,2 + 5]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three, four, five), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)
          ...
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   ...
                   "four" = colnames(df2[,2 + 4]),
                   "five" = colnames(df2[,2 + 5]),
                   "six" = colnames(df2[,2 + 6]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three, four, five, six), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)
          ...
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   ...
                   "five" = colnames(df2[,2 + 5]),
                   "six" = colnames(df2[,2 + 6]),
                   "seven" = colnames(df2[,2 + 7]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three, four, five, six, seven), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)      
          ...
          df3 <- df2 %>% 
            rename("one" = colnames(df2[,2 + 1]),
                   ...
                   "six" = colnames(df2[,2 + 6]),
                   "seven" = colnames(df2[,2 + 7]),
                   "eight" = colnames(df2[,2 + 8]),
                   )
          
          df_num <- df3 %>% 
             rowwise() %>% 
             mutate(sum = mean(c(one, two, three, four, five, six, seven, eight), na.rm = T)) %>%   
             pivot_longer(!c(years, ELECTION, sum), names_to = "number", values_to = "won") %>% 
             filter(!is.na(won)) %>% 
             arrange(years)
    ...
    dm <- matrix.please(df3)
    
    df_num2 <- df_num %>% 
      filter(!won == FALSE)
    
    comm2000.2018_all_test <- comm2000.2018_all %>% 
      filter(county_id == county, district == dists[i]) %>% 
      arrange(year, desc(candidate_percent_vote)) %>% 
      group_by(year) %>% 
      mutate(place = ifelse(lag(winner) == TRUE, 2,
                           ifelse(lag(lag(winner)) == TRUE, 3, 
                                   ifelse(lag(lag(lag(winner))) == TRUE, 4, 
                                          ifelse(lag(lag(lag(lag(winner)))) == TRUE, 5, 
                                                 ifelse(lag(lag(lag(lag(lag(winner))))) == TRUE, 6, 
                                                        7)))))) %>% 
      mutate(place = ifelse(is.na(place), 1, place)) %>% 
      ungroup()

    incumbents <- comm2000.2018_all_test %>% 
      filter(winner == TRUE) %>%   
      mutate(incumbent = ifelse(candidate_name == lag(candidate_name), TRUE, FALSE)) %>% 
      select(county_id, district, year, candidate_name, incumbent) %>% 
      mutate(prev.inc = ifelse(incumbent == FALSE, lag(candidate_name), NA))

    for.summary <- comm2000.2018_all_test %>% 
      full_join(incumbents, by = c("county_id", "district", "year", "candidate_name")) %>% 
      group_by(year) %>% 
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      ...
      mutate(prev.inc = ifelse(is.na(prev.inc), lag(prev.inc), prev.inc)) %>% 
      ungroup() %>% 
      mutate(inc.unseated = ifelse(prev.inc == candidate_name, TRUE, FALSE)) %>% 
      group_by(year) %>% 
      mutate(inc.unseated = ifelse(any(inc.unseated) == TRUE, TRUE, FALSE)) %>% 
      ungroup() %>% 
      filter(winner == TRUE) %>% 
      mutate(inc.unseated = ifelse(is.na(inc.unseated) & !is.na(prev.inc) == TRUE, FALSE, 
                                   inc.unseated))

    reelected.num <- nrow(for.summary %>% filter(is.na(inc.unseated))) - 1
    retired.num <- nrow(for.summary %>% filter(inc.unseated == FALSE)) 
    incumbent.unseat.num <- nrow(for.summary %>% filter(inc.unseated == TRUE)) 
    
    county_dist_stats_list[[i]] <- tibble(county_id = county, district = dists[i],
           num.elections = nrow(df_num2),
           diff.cands = df_num2 %>% distinct(number) %>% nrow(),
           eval.elections = nrow(for.summary),
           reelected = reelected.num,
           retired = retired.num,
           incumbent.unseat = incumbent.unseat.num)
    
  }
    
  county_dist_stats[[k]] <- do.call(bind_rows, county_dist_stats_list)
  
}

all_county_dist_stats <- do.call(bind_rows, county_dist_stats)
    
```

```{r}
county_pop_90_19 <- read_xlsx("relevant_county_data/county_population_1990-2019.xlsx",
                              skip = 2,
                              col_names = c("geo_type", "county_name", "year",
                                            "population", "num_households",
                                            "persons_per_household")) %>%
                        filter(!geo_type %in% c("EDR", "State")) %>% 
                        select(county_name:persons_per_household) %>% 
                        full_join(county_ids, by = c("county_name")) %>% 
                        mutate(county_id = as.factor(county_id))

all_county_dist_stats %>% 
  mutate(county_id = as.factor(county_id),
         district = as.factor(district)) %>% 
  full_join(comm_dists_19_area_final %>% 
              select(!geometry) %>% 
              rename(district = ctycomdist), by = c("county_id", "district")) %>% 
  full_join(county_pop_90_19 %>% filter(year == 2011), by = c("county_id", "county_name",
                                                              "region", "num_ctycomdists")) %>% 
  mutate(num.elections = as.factor(num.elections)) %>% 
  mutate(avg.pop = population/num_ctycomdists) %>%
  mutate(log.avg.pop = log(avg.pop)) %>% 
#  filter(num_ctycomdists == 7) %>% 
#  filter(population <= 600000) %>% 
#  lm(data = ., diff.cands ~ avg.pop) %>% summary()
  ggplot(aes(x = log.avg.pop, y = diff.cands)) + 
  geom_jitter(aes(color = num.elections), width = 0.0, height = 0.15, alpha = 0.85) + 
  geom_smooth(method = "lm", color = "firebrick2", se = TRUE) +
  scale_color_viridis(discrete = TRUE, option = "E", name = "Elections") +
  labs(title = "Unique County Commissioners by District from 2000-2018",
       #subtitle = "Distributed by Average District Population [excluding counties with 7 districts]",
       subtitle = "Distributed by Average District Population",
       x = "Log average county commissioner district population",
       y = "Unique commissioners 2000-2018") + 
  theme_minimal() +
...
all_county_dist_stats %>% 
  ...
  full_join(county_pop_90_19 %>% filter(year == 2011), by = c("county_id", "county_name",
                                                              "region", "num_ctycomdists")) %>% 
  mutate(num.elections = as.factor(num.elections)) %>% 
  group_by(county_id, num_ctycomdists, population, region) %>% 
  summarize(cands.sum = sum(diff.cands)) %>% 
  ungroup() %>% 
  mutate(cands.avg = cands.sum/num_ctycomdists) %>% 
  mutate(avg.pop = population/num_ctycomdists) %>%
  mutate(log.avg.pop = log(avg.pop)) %>%  
#  filter(avg.pop <= 60000) %>% 
#  filter(num_ctycomdists != 7) %>% 
#  lm(data = ., cands.avg ~ avg.pop) %>% summary()
  ggplot(aes(x = log.avg.pop, y = diff.cands)) + 
  geom_jitter(aes(color = region), width = 0, height = 0, alpha = 0.7) + 
  geom_smooth(method = "lm", color = "firebrick2", se = TRUE) +
  scale_color_viridis(discrete = TRUE, option = "D", name = "Region") +
  labs(title = "Unique County Commissioners by District from 2000-2018",
       #subtitle = "Distributed by Average District Population [excluding counties with 7 districts]",
       #subtitle = "Distributed by Average District Population",
       x = "average county commissioner district population",
       y = "unique commissioners 2000-2018") + 
  theme_minimal() +
...
all_county_dist_analyze <- all_county_dist_stats %>%
  mutate(county_id = as.factor(county_id),
         district = as.factor(district)) %>% 
  full_join(comm_dists_19_area_final %>% 
              select(!geometry) %>% 
...
#  mutate(num.elections = as.factor(num.elections)) %>% 
  mutate(avg.pop = population/num_ctycomdists) %>%
  mutate(log.avg.pop = log(avg.pop)) %>% 
#  filter(!num.elections == "4") %>% 
  mutate(avg.dens = avg.pop/area)
all_county_dist_analyze %>% 
  ggplot(aes(x = log.avg.pop, y = diff.cands)) + 
  geom_jitter(height = 0.1, width = 0.05, aes(color = num.elections)) + 
  geom_smooth(method = "lm")

lm(data = all_county_dist_analyze, diff.cands ~ log.avg.pop + num.elections) %>% summary()
...
ncol(dm)
    ...
    dm_smry <- data.matrix(summary(dm[,2]))
    ran <- nrow(dm) - dm_smry[[7,1]] #times ran
    won <- (nrow(dm) - dm_smry[[7,1]])*dm_smry[[4,1]] #times won
    lost <- nrow(dm) - dm_smry[[7,1]] - won #times lost
    tibble(ran = ran, won = won, lost = lost)
...
## County areas (must run before above plots)

comm_dists_19_area <- comm_dists_19 %>%
  mutate(area = as.numeric(st_area(geometry))/2.59e+6) %>% # convert to acres
  as_tibble()

comm_dists_19_area2 <- comm_dists_19_area %>% 
  full_join(comm_dists_19_area %>%
              group_by(county_id) %>% 
              top_n(1, area) %>% 
              mutate(density_rank = 1),
          by = c("county_name", "county_id", "ctycomdist", 
                 "geometry", "area")) %>% 
  full_join(comm_dists_19_area %>%
              group_by(county_id) %>% 
              top_n(2, area) %>% 
              top_n(-1, area) %>% 
              mutate(density_rank = 2),
          by = c("county_name", "county_id", "ctycomdist", 
                 "geometry", "area")) %>% 
  mutate(density_rank = coalesce(density_rank.x, density_rank.y)) %>% 
  select(county_name, county_id, ctycomdist, geometry, area, density_rank) %>% 
  full_join(comm_dists_19_area %>%
              group_by(county_id) %>% 
              top_n(3, area) %>% 
              top_n(-1, area) %>% 
              mutate(density_rank = 3),
          by = c("county_name", "county_id", "ctycomdist", 
                 "geometry", "area")) %>% 
  mutate(density_rank = coalesce(density_rank.x, density_rank.y)) %>% 
  select(county_name, county_id, ctycomdist, geometry, area, density_rank) %>% 
  ...
  full_join(comm_dists_19_area %>%
              ...
              top_n(5, area) %>% 
              top_n(-1, area) %>% 
              mutate(density_rank = 5),
          by = c("county_name", "county_id", "ctycomdist", 
                 "geometry", "area")) %>% 
  mutate(density_rank = coalesce(density_rank.x, density_rank.y)) %>% 
  select(county_name, county_id, ctycomdist, geometry, area, density_rank)

comm_dists_19_area_final <- comm_dists_19_area2 %>% 
  full_join(comm_dists_19_area2 %>%
              filter(is.na(density_rank)) %>%  
              group_by(county_id) %>% 
              top_n(1, area) %>% 
              mutate(density_rank = 6),
          by = c("county_name", "county_id", "ctycomdist", 
                 "geometry", "area")) %>% 
  mutate(density_rank = coalesce(density_rank.x, density_rank.y)) %>% 
  select(county_name, county_id, ctycomdist, geometry, area, density_rank) %>% 
  mutate(density_rank = ifelse(is.na(density_rank), 7, density_rank)) %>% 
  full_join(county_ids %>% 
              mutate(county_id = as.factor(county_id)), 
            by = c("county_id", "county_name")) %>% 
  mutate(density_rank = as.factor(density_rank)) %>% 
  mutate(jenks_class = case_when(area < 128.2340398 ~ 1,
                                 area < 304.7857670 ~ 2,
                                 area < 596.3743495 ~ 3,
                                 area < 1108.2014783 ~ 4,
                                 area < 2136.4647084 ~ 5,
                                 area < 4319.1657454 ~ 6,
                                 TRUE ~ 7)) %>% 
  mutate(jenks_class = as.factor(jenks_class)) %>% 
  mutate(jenks_class = case_when(area < 128.2340398 ~ 1,
                                 area < 304.7857670 ~ 2,
                                 area < 596.3743495 ~ 3,
                                 area < 1108.2014783 ~ 4,
  ...
  mutate(jenks_class = as.factor(jenks_class)) %>% 
  arrange(area) %>% 
  mutate(fifth = c(rep(1:5, each = 89), 5, 5)) %>%
  mutate(fifth = as.factor(fifth)) 
...
comm_dists_10 <- results_10 %>% 
  ...
  summarize(geometry = st_union(geometry)) %>% 
  ungroup() %>% 
  fill_holes(threshold = units::set_units(0.68, miles^2)) %>% 
