...
county_stats_2002 <- function(county_id){
  ...
  tibble(headers = head(as.vector(html_text(html_nodes(read_html(url), 
                                                              'span.boldFont'))), 3),
...
county_unclean_2002_general <- function(county_id){
  ...
  table_draft <- data %>% mutate(every_other = rep(c("candidate_votes", "candidate_percent_vote"), 
    ...
    mutate(party_affiliation = ifelse(grepl("WRITE-IN", candidate_name), "WI", "NP")) %>% 
    select(county_id, candidate_name, party_affiliation, num_precincts_reporting,
           total_num_precincts_voting, candidate_votes, candidate_percent_vote, unique_num) 
  ...
  allocate <- tibble(count = differ,
                     cand = tail(as.vector(html_text(html_nodes(read_html(url), 
                                                                'span.boldFont'))), leng - 6))
  allo_column <- as.tibble(as.vector(unlist(mapply(rep, allocate$cand, allocate$count))))
  
  cbind(allo_column, table_draft)
...
county_id_numbers <- tibble(county_id = 21:25) %>% 
  mutate(error = map_lgl(county_id, test_for_errors))
county_id_numbers


county_unclean_2002_general(25)
