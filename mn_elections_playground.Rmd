...
county_id <- 4
  ...
  url <- paste0("https://electionresults.sos.state.mn.us/results/Index?ErsElectionId=32&scenario=LocalCounty&CountyId=", county_id)
...
county_unclean_2002_general <- function(county_id){
  ...
  table_draft <- data %>% mutate(every_other = rep(c("candidate_votes", "candidate_percent_vote"), 
                                    repeat_this/2),
    ...
    select(county_id, candidate_name, party_affiliation, county_num_precincts_reporting,
           county_total_num_precincts_voting, candidate_votes, candidate_percent_vote, unique_num) 
...
test_for_errors <- function(county_id){
  
  is.error(county_unclean_2002_general(county_id))
  
}
  
#county_id_numbers <- tibble(county_id = 61:87) %>% 
#  mutate(error = map_lgl(county_id, test_for_errors))
#county_id_numbers

#county_stats_2002(4)

#county_unclean_2002_general(4)
...
county_stats_2000 <- function(county_id){
  url <- paste0("https://electionresults.sos.state.mn.us/results/Index?ErsElectionId=22&scenario=LocalCounty&CountyId=", county_id)
...
#county_id_numbers <- tibble(county_id = 64:87) %>% 
#  mutate(error = map_lgl(county_id, test_for_errors))
#county_id_numbers

#county_stats_2000(4)
  
#county_unclean_2000_general(4)

# Creating vector of counties ... will enter primaries manually
...
```{r}
  ...
  read.delim(paste0("relevant_county_data/county_races_datasets/", 
                    2004, "_county_", "gen", ".txt"), 
  sep = ";", header = FALSE, col.names = elec.col.names) %>% clean_names() %>% View
