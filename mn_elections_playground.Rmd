...
data2000.2018 %>% 
  filter(county_id == 85, office_name == "Commissioner") %>%
#  filter(year == 2010)
...
data2000.2018 %>% 
  ...
  select(candidate_name, county_id, district) %>%
  mutate(candidate_name = str_remove_all(candidate_name, "[.]")) %>% 
  mutate(candidate_name = str_replace_all(candidate_name, "  ", " ")) %>%
  mutate(candidate_name = str_replace_all(candidate_name, "   ", " ")) %>%
  mutate(candidate_name = str_remove_all(candidate_name, "[.]")) %>% 
  ...
  mutate(candidate_name = ifelse(candidate_name == "Walter Mianowski", "Walt Mianowski",
                                 candidate_name)) %>%
  distinct(candidate_name, county_id, last_name) %>% 
  filter(!last_name == "(Total)") %>% 
  filter(!candidate_name == "Write In") %>% 
  get_dupes(last_name) %>% 
  arrange(last_name, county_id) %>% View
...
county <- 1
  ...
  one <- clean_county(years[1], "general") %>% 
    ...
    select(district, candidate_name_pre, candidate_votes_pre, 
           candidate_percent_vote_pre) 
  
  two <- clean_county(years[2], "general") %>% 
    filter(county_id == county) %>% 
    mutate(candidate_name = str_remove_all(candidate_name, "[.]")) %>% 
...
# DRAFT 
compare_county_candidates <- function(county_id, years){
