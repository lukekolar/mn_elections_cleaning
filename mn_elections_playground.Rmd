...
clean_county <- function(year = c(2000:2020), type = c("general", "primary")){
  ...
  read.delim(paste0("county_races_datasets/", year, "_county_", election, ".txt"), 
...
clean_county(2010, "general") %>% filter(county_id == 18) %>% 
...
county_gen_2006 <- read.delim("county_races_datasets/2006_county_gen.txt", 
  sep = ";", header = FALSE, col.names = elec.col.names) %>% clean_names() %>% 
  select(county_id, office_id:candidate_name,
         total_num_precincts_voting:total_race_votes) %>% 
  mutate(office_name = str_to_title(office_name)) %>%
...
county_gen_2014 <- read.delim("county_races_datasets/2014_county_gen.txt", 
  ...
  filter(!grepl("COUNTY QUESTION", office_name)) %>% 
  mutate(office_name = )
