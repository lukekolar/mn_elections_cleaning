...
pres_elections_01 %>% filter(county_name == "Saint Louis")
#
...
comm_dists_01 <- vtd_01 %>% 
  ...
  mutate(ctycomdist = ifelse(ctycomdist == 8, 
                             case_when(county_name == "Aitkin" ~ 8, #complex...
                                       ...
                                       county_name == "Beltrami" ~ 4, #also a zero!!
                                       county_name == "Benton" ~ 2,
                                       county_name == "Blue Earth" ~ 8, #correction needed.
                                       ...
                                       county_name == "Lake of the Woods" ~ 8, #complex,lake zero!!
                                       county_name == "Le Sueur" ~ 5,
                                       county_name == "Marshall" ~ 5,
                                       county_name == "Mower" ~ 8, #correction needed
                                       county_name == "Pope" ~ 3,
...
comm_dists_01 %>% filter(ctycomdist %in% c(0, 8))


grid.arrange(plot_com_elections(2004, "general", "Aitkin"), 
             plot_com_elections(2012, "general", "Aitkin"), nrow = 1)
```


```{r}

unique(comm_dists_01_mod$precinct_name) %>% distinct(precinct_name) %>% nrow()

comm_dists_01_mod %>% 
  filter(county_name == "Aitkin") %>% 
#  filter(ctycomdist == 8)
  mutate(ctycomdist = ifelse(ctycomdist == 8, 
                             case_when(county_name == "Aitkin" ~
                                case_when(precinct_name == "Northeast Aitkin Unorg." ~ 5,
        ...
        !!                                  precinct_name == "Northwest Aitkin Unorg." ~ 5,
                                          ...
                                          TRUE ~ 8),
                                       county_name == "Cook" ~
                                case_when(precinct_name == "Maple Hill P-4" ~ 4,
                             ...
                             ctycomdist)) %>% 
  mutate(ctycomdist = as.factor(ctycomdist)) %>% 
  ggplot(aes(fill = ctycomdist)) + geom_sf()


comm_dists_01_mod %>% 
  filter(county_name == "Cook", precinct_name == "Lutsen P-8"
         ) %>% 
# filter(ctycomdist == 8) 
#   filter(!precinct_name %in% c("Maple Hill P-4", "Grand Marais East P-5")) %>% 
  mutate(ctycomdist = as.factor(ctycomdist)) %>% 
  ggplot(aes(fill = ctycomdist)) + geom_sf()

grid.arrange(plot_com_elections(2006, "general", "Cook"), 
             plot_com_elections(2012, "general", "Cook"), nrow = 1)
...
grid.arrange(
  ggplot(comm_dists_01 %>% filter(county_name == countynam, ctycomdist %in% c(1)), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
    theme(legend.position = "none"),
  ggplot(comm_dists_01 %>% filter(county_name == countynam, ctycomdist %in% c(2)), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
    theme(legend.position = "none"),
  ggplot(comm_dists_01 %>% filter(county_name == countynam, ctycomdist %in% c(3)), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
    theme(legend.position = "none"),
  ggplot(comm_dists_01 %>% filter(county_name == countynam, ctycomdist %in% c(4)), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
    theme(legend.position = "none"),
  ggplot(comm_dists_01 %>% filter(county_name == countynam, ctycomdist %in% c(5)), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
    theme(legend.position = "none"), 
  ggplot(comm_dists_01 %>% filter(county_name == countynam), 
         aes(fill = ctycomdist)) + geom_sf() + scale_fill_viridis_d() + 
...
cny_94_raw <- readOGR( 
  dsn = paste0(getwd(),"/general_data/94_precincts/"), 
  layer = "Counties_DD",
  verbose = FALSE)

proj4string(cny_94_raw) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")

cny_94_messy <- spTransform(cny_94_raw, 
            CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))

cny_94 <- st_as_sf(cny_94_messy) %>% 
  clean_names()

cny_94 %>% head(40) %>% View

ggplot(cny_94) + geom_sf()
