...
```{r, eval = F, include = F}
## Creating county commissioner district polygons: 2002-2011 ... DO NOT RUN

# note: data has since been deleted from local folder...

### voting_dists_11_raw <- readOGR( 
###   dsn = paste0(getwd(),"/general_data/shp_society_redistricting_2010_large/"), 
###   layer = "blk2010",
###   verbose = FALSE)

### save(voting_dists_11_raw, file="general_data/voting_dists_11_raw_blk.rdata")

### voting_dists_11_messy <- spTransform(voting_dists_11_raw, 
###             CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))

### save(voting_dists_11_messy, file="general_data/voting_dists_11_messy_blk.rdata")

### voting_dists_11 <- st_as_sf(voting_dists_11_messy) %>% clean_names()

### save(voting_dists_11, file="general_data/voting_dists_11_blk.rdata")
...
test <- voting_dists_11 %>%
  st_make_valid() %>% 
  group_by(county, countyld) %>%
  summarize(geometry = st_union(geometry)) %>% 
  ungroup()
head(test, 5)
  mutate(countyname = fct_explicit_na(countyname)) %>%
