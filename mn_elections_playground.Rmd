...
vtd_01 %>% 
  ...
  mutate(null_votes = ifelse(index_vote %in% c(0.0000, NA), TRUE, FALSE)) %>% 
  rename(precinct_id = number,
         county_num = county,
         county_name = county_nam,
         precinct_name = name) %>% 
  select(precinct_id, county_num, county_name, precinct_name, geometry, null_votes) %>%
